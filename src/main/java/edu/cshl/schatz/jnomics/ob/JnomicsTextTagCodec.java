 ...
 *
 * Original Source found in Picard Toolkit
 */
class JnomicsTextTagCodec {
    ...
    String encode(final String tagName, Object value) {
        ...
        sb.append(':');
        char tagType = JnomicsBinaryTagCodec.getTagValueType(value);
        switch (tagType) {
            case 'c':
            case 'C':
            case 's':
    ...
    Map.Entry<String, Object> decode(final String tag) {
        ...
        if (numFields != JnomicsTextTagCodec.NUM_TAG_FIELDS) {
    ...
    private Object covertStringArrayToObject(final String stringVal) {
        ...
        switch (Character.toLowerCase(elementType)) {
            case 'c': {
                ...
                if (isUnsigned) return new JnomicsTagValueAndUnsignedArrayFlag(array, true);
                else return array;
            }
            case 's': {
